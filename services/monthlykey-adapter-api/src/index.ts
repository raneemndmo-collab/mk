/**
 * Monthly Key Adapter API
 *
 * Thin adapter between Monthly Key frontend and Hub API.
 * Enforces brand=MONTHLYKEY on all requests.
 * In standalone mode: proxies to Hub API.
 * In integrated mode: can also query Beds24 for real-time data.
 */
...
const MODE = process.env.MODE_MONTHLYKEY ?? "standalone";
...
app.get("/api/v1/units", async (req, res) => {
  try {
    const params = new URLSearchParams({ ...req.query as any, brand: "MONTHLYKEY" });
...
app.post("/api/v1/bookings/quote", async (req, res) => {
  try {
    const body = { ...req.body, brand: "MONTHLYKEY" };
...
app.listen(PORT, () => {
  logger.info(`Monthly Key Adapter API running on port ${PORT} (mode: ${MODE})`);
});
