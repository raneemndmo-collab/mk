          ...
          <div className="md:col-span-4 flex flex-col items-center md:items-start">
            ...
            {(twitter || instagram || linkedin) && (
              <div className="flex gap-3 mt-6">
                {twitter && (
                  <a href={twitter} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-white/40 hover:text-[#3ECFC0] hover:bg-[#3ECFC0]/10 hover:border-[#3ECFC0]/30 transition-all duration-300">
                    <svg className="h-3.5 w-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                  </a>
                )}
                {instagram && (
                  <a href={instagram} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-white/40 hover:text-[#3ECFC0] hover:bg-[#3ECFC0]/10 hover:border-[#3ECFC0]/30 transition-all duration-300">
                ...
                {linkedin && (
                  <a href={linkedin} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-white/40 hover:text-[#3ECFC0] hover:bg-[#3ECFC0]/10 hover:border-[#3ECFC0]/30 transition-all duration-300">
      ...
      {hasLicences && (
        <div className="border-t border-white/5">
          <div className="container py-4">
            <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2 text-[10px] text-white/25">
              {tourismLicence && (
                <div className="flex items-center gap-1">
                  <Shield className="h-3 w-3 text-[#C9A96E]/40" />
                  <span>{lang === "ar" ? "ترخيص وزارة السياحة:" : "Tourism Licence:"}</span>
                  <span className="text-white/40 font-medium" dir="ltr">{tourismLicence}</span>
                </div>
              )}
              {crNumber && (
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-white/40 font-medium" dir="ltr">{crNumber}</span>
                </div>
              )}
              {vatNumber && (
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-white/40 font-medium" dir="ltr">{vatNumber}</span>
                </div>
              )}
              {ejarLicence && (
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-white/40 font-medium" dir="ltr">{ejarLicence}</span>
                </div>
              )}
            </div>
          </div>
